...
void* thread1()
{
	...
	while(1)
	{
		...
		pthread_mutex_lock(&lock1);
		printf("Thread1 : lock1 acquired\n");
		printf("Thread1 : Waiting to acquire lock2\n");
		pthread_mutex_lock(&lock2);
		printf("Thread1 : lock2 acquired\n");

		manipulate(1);
		printf("Thread1 : Total_2 = %d\n", Total_1);
		printf("Thread1 : Total_1= %d\n", Total_2);

		printf("Thread1 : Realising lock 2\n");
		pthread_mutex_unlock(&lock2);
		printf("Thread1 : Realising lock 1\n");
		pthread_mutex_unlock(&lock1);
	}
}
...
int main()
{
	...
	if(pthread_create(&thread_id, NULL, thread2, NULL) != 0)

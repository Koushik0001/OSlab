...
***********************************************************************************************************************
    ...
    Solution :
			...
			7) When threadA becomes unblocked again next time the 'previousThread' variable is set to 'b'.
			...
			9) When the queue beomes empty the isEmpty() call returns 1 and 'threadA' stops exectuing the while loop.
			   ...
			   thradA now finishes execution and joins the default thread.
			...
			11) After finishing execution all the three threads finishes causing the pthread_join() calls to return and the
...
***********************************************************************************************************************
    INPUT DESCRIPTION : 
			1) Input can be given using commandline arguments. Number of commandline arguments (if provided) must be even.
			...
			3) Choosing option 2 while the program prompts for a choice will display the contents of the queue.
			4) Choosing option 3 will make thr program to process the contents of the queue using the threads.

    OUTPUT DESCRIPTION : 

        	1)	This program prints the results of the execution of the threads on the (x, y) pairs as a table.
				Where output of threadA : (X*Y)
								threadB : (X*Y)/2
								threadC : (X+Y)
								threadB : ((X*Y) /(X+Y))
								threadA : ((X+Y)* (X+Y))
				Floating point numbers are printed upto 3 decimal positions
				example : 
						Pairs  | A | B | C |   B   | A
						------------------------------
						(1, 2) | 2 | 1 | 3 | 0.667 | 9
...
***********************************************************************************************************************
    EXECUTION :
        ...
        Execution Sequence      :  	./19_8 [x y] ... or,
									./19_8
									(arguments are optional but must be in even numbers when provided)
...
void handleCommandLineInputs(int argc, char **argv); //this function reads the commandline inputs and inserts them in the queue as (x, y) pairs
...
int main(int argc, char **argv)
{
	...
	handleCommandLineInputs(argc, argv);
...
void* threadA(void* args)
{
	while (!isEmpty()) /*run until queue is empty*/
	{
		...
		//waiting on semaphoreA to become>0
		...
		if (previousThread == 'o')
		{
			//get the x and y values from the first node of the queue
			dequeue(&x, &y);
			printPair(x, y);
			...
			//set previousThead to 'a' threadA is the most recent thread to run
			previousThread = 'a';
			
			//unblock threadB which will process the (x, y) pair next
		...
		else if (previousThread == 'b')
		{
			...
			//delete the front or first node of the queue
            delete();

			//set previousThread to 'o' because processing of an element is done by all the three queues and
			//a new cycle will begin with the next element of the queue and threadA
			previousThread = 'o';

			//increament semaphoreA so that threadA doesn't block when sem_wait(&semaphoreA) is executed next in the begining
			//of the new cycle
...
void* threadB(void* args)
{
	while (1)
	{
		...
		//if last thread to execute was threadA run this if block
		if (previousThread == 'a')
		{
            dequeue(&x, &y);
		...
		//if last thread to execute was threadB run this block of code
		...
		//if the queue is empty then run this to break out of the while loop
        if(isEmpty())
            break;
	}
}
...
void handleCommandLineInputs(int argc,char **argv)
{
